        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('protiles.store') }}" enctype="multipart/form-data" method="post">
                        <div class="row mb-4">
                                ...
                                <select name="boyuts[]" class="form-control" multiple>
                                    @foreach($boyut as $size)
                                        <option value="{{ $size->id }}"
                                                @if(is_array(old('boyuts', $ekibimiz->boyuts ?? [])) && in_array($size->id, old('boyuts', $ekibimiz->boyuts ?? [])))
                                                    selected
                                            @endif>
                                            {{ $size->title }}
                                        </option>
                            ...
                            <div class="col-sm-9">
                                <select name="types[]" class="form-control" multiple>
                                    @foreach($type as $size)
                                        <option value="{{ $size->id }}"
                                                @if(is_array(old('types', $ekibimiz->types ?? [])) && in_array($size->id, old('types', $ekibimiz->types ?? [])))
                                                    selected
                                            @endif>
                                            {{ $size->title }}
                                        </option>
